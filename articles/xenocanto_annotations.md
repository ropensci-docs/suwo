...
details.suwo (version \>= 0.2.3) now allows users to retrieved
annotations along with the associated sound files.
...
species in these cases as `Sonus naturalis` (not a real biological
taxon, but a placeholder Xeno-Canto uses specifically for this recording
type). This can show up unexpectedly when inspecting annotation species,
even for a search targeting a real species:
...
```
    ...
    ## 
    ##    Corvus corax Sonus naturalis 
    ##               2             151

Keep this in mind when filtering or summarizing annotations by species.
To search specifically for this kind of recording, use `grp:soundscape`
(or `grp:0`). See [Searching non-bird taxa](#searching-non-bird-taxa)
below.
...
specific annotation attribute. By default this will return only bird
recordings, since Xeno-Canto is primarily a bird sound repository.
...
To search for annotated recordings in other groups it must be combined
with `grp:`:
...
manipulation and analysis. The following function formats annotations to
meet warbleR’s requirements. It takes the full output of
[query_xenocanto()](https://docs.ropensci.org/suwo/reference/query_xenocanto.html)
and returns a data frame with the columns required by
...
```
    ...
    ##       key annotation_xc_id            species    annotator  start    end
    ## 1 1104794             1488 Gryllus campestris Burooj Ghani 0.0160 1.1338
    ## 2 1104794             1489 Gryllus campestris Burooj Ghani 1.4904 4.3828
    ##   top.freq bottom.freq sound_type  sex life_stage annotation_remarks
    ## 1   15.639       3.524       <NA> <NA>       <NA>                 NA
    ## 2   16.147       4.167       <NA> <NA>       <NA>                 NA
    ##            original_set_name original_set_creator original_set_owner
    ## 1 Annotation set for 1104794         Burooj Ghani               <NA>
    ## 2 Annotation set for 1104794         Burooj Ghani               <NA>
    ##   original_set_license                          original_set_uri
    ## 1            CC-BY-4.0 https://xeno-canto.org/annotation/set/102
    ## 2            CC-BY-4.0 https://xeno-canto.org/annotation/set/102
    ##   original_set_creation_date          annotation_set_name
    ## 1        2026-05-02 23:52:40 Annotation set for XC1104794
    ## 2        2026-05-02 23:52:40 Annotation set for XC1104794
    ##   annotation_set_creator annotation_set_creation_date
    ## 1                   <NA>          2026-09-10 17:21:00
    ## 2                   <NA>          2026-09-10 17:21:00
    ##                                                    annotation_set_remarks
    ## 1 Metadata of the original set is provided for each individual annotation
    ## 2 Metadata of the original set is provided for each individual annotation
    ##                                  file_url                observation_url
    ## 1 https://xeno-canto.org/1104794/download https://xeno-canto.org/1104794
    ## 2 https://xeno-canto.org/1104794/download https://xeno-canto.org/1104794
    ##   file_extension       main.species                      sound.files
    ## 1            wav Gryllus campestris Gryllus_campestris-XC1104794.wav
    ## 2            wav Gryllus campestris Gryllus_campestris-XC1104794.wav

These formatted annotations can be used with
[warbleR](https://CRAN.R-project.org/package=warbleR),
[baRulho](https://CRAN.R-project.org/package=baRulho), or
[ohun](https://CRAN.R-project.org/package=ohun) functions to visualize
and analyze the annotated segments once the sound files have been
downloaded with suwo’s
